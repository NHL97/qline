...
<body>
    ...
    <div class="card">
        ...
        @if (session('message'))
            <div class="flash">
                <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="#0f766e" stroke-width="2"
        ...
        @if ($business->queue_status === 'paused' && $business->pause_reason)
            <div
                style="margin-bottom:16px; padding:10px 14px; background:rgba(234,179,8,0.08); border:0.5px solid rgba(234,179,8,0.2); border-radius:10px; font-size:13px; color:#92400e; display:flex; align-items:center; gap:8px;">
                <span>⏸</span>
                <span>{{ $business->pause_reason }}</span>
            </div>
        @endif
        ...
        @if (in_array($entry->status, ['called', 'serving']))
            <div class="message message-serving">
                <div class="message-icon icon-serving">
                    <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="#0f766e"
                        stroke-width="2" stroke-linecap="round">
                        <path d="M8 2a6 6 0 110 12A6 6 0 018 2z" />
        ...
        @if ($entry->status === 'cancelled' || $entry->status === 'skipped')
            <div class="message message-cancelled">
                <div class="message-icon icon-cancelled">
                    <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="#b91c1c"
                        stroke-width="2" stroke-linecap="round">
                ...
                if (window.Echo) {
                    ...
                    window.Echo.channel('queue.{{ $business->slug }}')
                        .listen('.queue.updated', (data) => {
                            // Only reload if queue status changed or entry is active
                            @if ($entry->isActive())
                                window.location.reload();
                            @else
                                // Entry is done/cancelled — only reload if queue status changed
                                if (data.queue_status !== '{{ $business->queue_status }}') {
                                    window.location.reload();
                                }
                            @endif
                        });
